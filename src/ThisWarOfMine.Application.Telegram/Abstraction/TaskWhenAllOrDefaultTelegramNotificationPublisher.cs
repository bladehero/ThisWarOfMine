...
internal sealed class TaskWhenAllOrDefaultTelegramNotificationPublisher : INotificationPublisher
{
    ...
    )
    {
        ...
        var handlers = allKnownExecutors.Select(x => x.HandlerInstance).Cast<ITelegramNotificationHandler>().ToArray();
        var nonDefaultHandlers = handlers.Where(handler => HandlerIsNotDefault(handler, notificationType)).ToArray();
    ...
    private static async Task<bool> ExecuteHandlerAsync(ITelegramNotificationHandler handler, CancellationToken token)
    {
