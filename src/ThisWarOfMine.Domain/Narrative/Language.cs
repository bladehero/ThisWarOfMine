...
public sealed class Language : SmartEnum<Language>
{
    ...
    public static Language FromShortName(string shortName) =>
        TryFromShortName(shortName).GetValueOrThrow($"Cannot find language by short name: `{shortName}`");

    public static Maybe<Language> TryFromShortName(string shortName) => List.TryFirst(x => x.ShortName == shortName);
}
